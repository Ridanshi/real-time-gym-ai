...
def main():
    ...
    with st.sidebar:
        ...
        if not workout_started:
            ...
            if end_session_button:
                ...
                total_reps_done = int(st.session_state.get("reps", 0) or 0)
                last_saved_sets = int(st.session_state.get("last_saved_sets_completed", 0) or 0)
                reps_per_set_val = int(st.session_state.get("reps_per_set", 0) or 0)
                already_saved_reps = last_saved_sets * reps_per_set_val
                unsaved_reps = max(0, total_reps_done - already_saved_reps)
                ...
                if unsaved_reps > 0:
                    ...
                    add_exercise(user_id, exercise, unsaved_reps, 0, time_taken)
    ...
    st.markdown(
        """
        <div class="gym-tag"><span class="dot"></span>AI-POWERED &middot; REAL-TIME &middot; GYM COACH</div>
        <div class="gym-title">AI Real-time GYM Coach</div>
        <div class="gym-subtitle">Real-time pose detection with proactive AI voice coaching</div>
        """,
